...
// 2020 / 10 / 2
//
// Modified by Q-engineering
// 2022 / 06 / 11
//
// Modified by V. Aicher
// 2022 / 06 / 28
//
...
{
    static const char* class_names[] = { "fall", "adl"};
    ...
    cv::imshow("Fall Detection", image);
}
...
int video_demo(NanoDet& detector, const char* path)
{
    ...
    {   try {
        ...
        Tbegin = Tend;
         if(f>0.0) FpsAr[((Fcnt++)&0x0F)]=1000.0/f;
         for(f=0.0, i=0;i<16;i++){ f+=FpsAr[i]; }
		putText(image, cv::format("FPS %0.2f", f/16),cv::Point(10,20),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(0, 0, 255));

        //add to the end of the list
		FPSarray.push_back(f/16.0);

		//show output
		//imshow("Fall Detection", image);
    ...
    std::ofstream fw("FPS.txt");  //ofstream is already defined as output
...
int main(int argc, char** argv)
{
    ...
    NanoDet detector = NanoDet("./nanodet-opt.param", "./nanodet-opt.bin", true);
